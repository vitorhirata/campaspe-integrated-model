...
@testset "#run_policy integration test" begin
    @testset "runs policy and returns valid allocations" begin
        # Setup test parameters
        data_path = "../data"
        model_run_range::StepRange{Date, Period} = Date("1970-07-01"):Day(1):Date("1971-06-30")
        goulburn_alloc_scenario = "high"
        ...
        daily_dam_release, farm_allocations = CampaspeIntegratedModel.run_policy(
            policy_state,
            ts,
            dt,
            f_orders,
