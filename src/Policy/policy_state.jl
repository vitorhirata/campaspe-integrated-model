...
# Returns
- `Tuple{Dict{String, Any}, DataFrame, DataFrame, DataFrame}` : (farm_zone_info, env_systems, other_systems, zone_shp)
  ...
  - zone_shp: processed GeoDataFrame with all calculated fields
"""
function zonal_info(zone_data_path::String)::Tuple{Dict{String, Any}, DataFrame, DataFrame, DataFrame}
    # Read farm zone shapefile
    farm_zone_shp_path = joinpath(zone_data_path, "farm_gda_94", "farm_gda94.shp")
    zone_shp = GeoDataFrames.read(farm_zone_shp_path)
    ...
    zone_shp = leftjoin(zone_shp, ag_area, on=:ZoneID)
    ...
    zone_shp_filtered = filter(row -> !ismissing(row.WatSystem), zone_shp)

    # Group by WatSystem to get max wat_HR (Campaspe Entitlement for each SW zone)
    water_systems = combine(groupby(zone_shp_filtered, :WatSystem), :wat_HR => maximum => :water_ent)

    # Group by WatSystem to sum agric_ha (Cropping area for each SW Zone)
    agri_area = combine(groupby(zone_shp_filtered, :WatSystem), :agric_ha => sum => :area)
    ...
    gw_area = combine(groupby(zone_shp, :TRADING_ZO), :zone_ha => sum => :zone_ha_sum)

    # Add calculated columns to zone_shp
    zone_shp[!, :watsys_agri_area] = zeros(nrow(zone_shp))
    zone_shp[!, :gw_Ent] = zeros(nrow(zone_shp))
    zone_shp[!, :prop_crop_area] = zeros(nrow(zone_shp))

    # Calculate proportional values for each zone
    for (idx, row) in enumerate(eachrow(zone_shp))
        if ismissing(row.WatSystem) || isnothing(row.WatSystem)
            zone_shp[idx, :watsys_agri_area] = 0.0
            ...
            if nrow(sys_match) > 0
                zone_shp[idx, :watsys_agri_area] = sys_match[1, :area]
            else
                zone_shp[idx, :watsys_agri_area] = 0.0
            end
        ...
        zone_shp[idx, :gw_Ent] = row.wat_GW * gw_proportional_area

        # Calculate proportional crop area
        prop_area = row.agric_ha / row.zone_ha
        if isinf(prop_area) || isnan(prop_area)
            zone_shp[idx, :prop_crop_area] = 0.0
        else
            zone_shp[idx, :prop_crop_area] = prop_area
        end
    end
    ...
    if !hasproperty(zone_shp, :Camp_HR_Ent)
        zone_shp[!, :Camp_HR_Ent] = zone_shp.wat_HR .* zone_shp.prop_crop_area
    end
    if !hasproperty(zone_shp, :Camp_LR_Ent)
        zone_shp[!, :Camp_LR_Ent] = zone_shp.wat_LR .* zone_shp.prop_crop_area
    end

    # Adjust Goulburn entitlements based on proportional crop area
    zone_shp[!, :goul_HR] = (zone_shp.agric_ha ./ zone_shp.zone_ha) .* zone_shp.goul_HR
    zone_shp[!, :goul_LR] = (zone_shp.agric_ha ./ zone_shp.zone_ha) .* zone_shp.goul_LR

    # Replace NaN values with 0
    zone_shp[!, :goul_HR] = coalesce.(zone_shp.goul_HR, 0.0)
    zone_shp[!, :goul_LR] = coalesce.(zone_shp.goul_LR, 0.0)

    # Add Goul_HR_Ent and Goul_LR_Ent if not present
    if !hasproperty(zone_shp, :Goul_HR_Ent)
        zone_shp[!, :Goul_HR_Ent] = zone_shp.goul_HR
    end
    if !hasproperty(zone_shp, :Goul_LR_Ent)
        zone_shp[!, :Goul_LR_Ent] = zone_shp.goul_LR
    end

    zone_shp[!, :Goul_HR_Ent] = coalesce.(zone_shp.Goul_HR_Ent, 0.0)
    zone_shp[!, :Goul_LR_Ent] = coalesce.(zone_shp.Goul_LR_Ent, 0.0)

    # Calculate total zone entitlements weighted by crop area
    zone_shp[!, :zone_crop_HR] = zone_shp.Camp_HR_Ent .+ zone_shp.Goul_HR_Ent
    zone_shp[!, :zone_crop_LR] = zone_shp.Camp_LR_Ent .+ zone_shp.Goul_LR_Ent
    ...
    return farm_zone_info, env_systems, other_systems, zone_shp
end
